...
import pandas as pd
import numpy as np
from analyzer import calculate_peer_stats, calculate_summary_score, create_peer_comparison_table
...
class TestAnalyzer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.peer_data = {
            'PEER1': {'P/E': 10, 'Forward P/E': 9, 'PEG Ratio': 1.0, 'EPS': 1.0, 'EPS Growth': 0.1, 'Debt/Equity': 50, 'ROE': 0.1, 'Operating Margin': 0.1, 'Gross Margin': 0.3, 'Revenue Growth': 0.05, 'Free Cash Flow': 1000},
            'PEER2': {'P/E': 20, 'Forward P/E': 18, 'PEG Ratio': 2.0, 'EPS': 2.0, 'EPS Growth': 0.2, 'Debt/Equity': 100, 'ROE': 0.2, 'Operating Margin': 0.2, 'Gross Margin': 0.5, 'Revenue Growth': 0.10, 'Free Cash Flow': 2000},
            'PEER3': {'P/E': None, 'Forward P/E': None, 'PEG Ratio': None, 'EPS': 1.5, 'EPS Growth': 0.15, 'Debt/Equity': 75, 'ROE': 0.15, 'Operating Margin': 0.15, 'Gross Margin': 0.4, 'Revenue Growth': 0.07, 'Free Cash Flow': 1500},
        }
        self.target_data = {'P/E': 15, 'Forward P/E': 14, 'PEG Ratio': 1.5, 'ROE': 0.25, 'Operating Margin': 0.25, 'Gross Margin': 0.6, 'Revenue Growth': 0.12, 'EPS Growth': 0.25, 'Debt/Equity': 40}
    ...
    def test_calculate_peer_stats(self):
        ...
        self.assertTrue(np.isnan(stats['PEG Ratio']['mean']) or isinstance(stats['PEG Ratio']['mean'], float))
    ...
    def test_calculate_summary_score_stronger(self):
        ...
        strong_target.update({'P/E': 8, 'ROE': 0.3, 'Debt/Equity': 20, 'Revenue Growth': 0.15})
    ...
    def test_calculate_summary_score_weaker(self):
        ...
        weak_target.update({'P/E': 30, 'ROE': 0.05, 'Debt/Equity': 200, 'Revenue Growth': 0.01})
        score = calculate_summary_score(weak_target, peer_stats)
        self.assertLess(score, 40)

    def test_calculate_summary_score_average(self):
        peer_stats = calculate_peer_stats(self.peer_data)
        avg_target = {k: v['mean'] for k, v in peer_stats.items() if 'mean' in v and pd.notna(v['mean'])}
        # Fill in missing keys for the test
