...
from report_generator import initialize_llm, create_full_prompt, generate_qualitative_analysis
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="AI Investor Agent for Stock Analysis")
    parser.add_argument("ticker", type=str, help="The stock ticker symbol to analyze (e.g., AAPL).")
    ...
    if not peers:
        print(f"⚠️ Could not find any peers for {target_ticker}. Analysis will be limited.")
    else:
    ...
    if target_ticker not in all_data:
        print(f"❌ Critical error: Failed to fetch data for the target ticker {target_ticker}. Exiting.")
        return
    ...
    llm = initialize_llm()
    if not llm:
        print("❌ Failed to initialize LLM. Cannot generate qualitative report.")
        # Proceed with just the quantitative data
        qualitative_analysis = "LLM analysis could not be generated."
    else:
        print("✅ LLM initialized. Creating prompt and generating report...")
        prompt = create_full_prompt(target_ticker, all_data[target_ticker], peer_analysis)
    ...
    # 1) Company Snapshot (from LLM)
    # We will parse the generated text to display it section by section
    # For simplicity here, we'll print the whole block. A more advanced version
    # would parse each section based on the headers.

    print("--- 1) Company Snapshot & Key Metrics ---")
    print(f"Name: {target_data.get('longName', target_ticker)}")
    print(f"Sector / Industry: {target_data.get('Sector', 'N/A')} / {target_data.get('Industry', 'N/A')}")
    ...
    # Print the rest of the LLM-generated report
